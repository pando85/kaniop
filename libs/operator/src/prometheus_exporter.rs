...
pub fn set_global_exporter(exporter: PrometheusExporter) {
    ...
    if let Ok(mut guard) = PROMETHEUS_EXPORTER.lock() {
        *guard = Some(exporter);
    }
}
...
pub fn format_prometheus_metrics(_service_name: &str) -> Result<String, String> {
    PROMETHEUS_EXPORTER
        .lock()
        .ok()
        .and_then(|guard| guard.as_ref().map(|e| e.get_metrics()))
        .flatten()
        .ok_or_else(|| "Prometheus exporter not initialized or poisoned".to_string())
}
