...
use kaniop_k8s_util::types::{compare_with_spn, normalize_spn};
...
pub struct KanidmOAuth2ClientSpec {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub claims_map: Option<BTreeSet<KanidmClaimsMap>>,
...
impl KanidmScopeMap {
    ...
    pub fn compare_with_entry(&self, entry: &str) -> bool {
        let parts = entry.split(':').collect::<Vec<_>>();
        if parts.len() != 2 {
            return false;
        }
        let group_spn = parts[0];
        if compare_with_spn(&self.group, group_spn).not() {
            return false;
        }
        if format!(" {:?}", self.scopes) != parts[1] {
            return false;
        }
        true
    }
...
impl KanidmClaimsMap {
    ...
    pub fn compare_with_entry(&self, entry: &str) -> bool {
        ...
        if self.join_strategy != join_strategy {
            return false;
        }

        if format!("\"{}\"", self.values.join(",")) != parts[3] {
            return false;
        }
...
#[cfg(test)]
mod tests {
    use crate::crd::KanidmClaimMapJoinStrategy;

    use super::{KanidmClaimsMap, KanidmScopeMap};
    ...
    #[test]
    fn test_kanidm_scope_map_compare_with_entry() {
        let scope_map = KanidmScopeMap {
            group: "test_group".to_string(),
            scopes: vec!["scope1".to_string(), "scope2".to_string()],
        };
        ...
        assert!(
            scope_map.compare_with_entry(r#"test_group@test.example.com: {"scope1", "scope2"}"#)
        );
        assert!(scope_map.compare_with_entry(r#"test_group: {"scope1", "scope2"}"#));
        assert!(scope_map
            .compare_with_entry(r#"test_group:{"scope1", "scope2"}"#)
            .not());
        assert!(scope_map
            .compare_with_entry(r#"test_group: {"scope1"}"#)
            .not());
    }

    #[test]
    fn test_kanidm_claim_map_from() {
        let mut claim_map = KanidmClaimsMap {
        ...
        assert_eq!(
            KanidmClaimsMap::from(r#"claim_name:group_name@test.example.com:;:"foo,boo""#)
                .unwrap()
                .normalize(),
            claim_map,
        );
        assert_eq!(
            KanidmClaimsMap::from(r#"claim_name:group_name:;:"foo,boo""#)
                .unwrap()
                .normalize(),
            claim_map,
        );
    ...
    #[test]
    fn test_kanidm_claim_map_compare_with_entry() {
        let mut claim_map = KanidmClaimsMap {
            name: "claim_name".to_string(),
            group: "group_name".to_string(),
            join_strategy: KanidmClaimMapJoinStrategy::Array,
        ...
        assert!(
            claim_map.compare_with_entry(r#"claim_name:group_name@test.example.com:;:"foo,boo""#)
        );
        assert!(claim_map.compare_with_entry(r#"claim_name:group_name:;:"foo,boo""#));
        assert!(claim_map
            .compare_with_entry(r#"claim_name:group_nam:;:"foo,boo""#)
            .not());
        assert!(claim_map
            .compare_with_entry(r#"claim_name:foo@test.example.com:;:"foo,boo""#)
            .not());
        assert!(claim_map
            .compare_with_entry(r#"fake:group_name:;:"foo,boo""#)
            .not());
        assert!(claim_map
            .compare_with_entry(r#"claim_name:group_name:;:"foo""#)
            .not());

        claim_map.join_strategy = KanidmClaimMapJoinStrategy::Csv;
        assert!(claim_map.compare_with_entry(r#"claim_name:group_name:,:"foo,boo""#));
        assert!(claim_map
            .compare_with_entry(r#"claim_name:group_name:':"foo,boo""#)
            .not());
