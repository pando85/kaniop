use crate::crd::{KanidmOAuth2Client, KanidmOAuth2ClientStatus};

use kaniop_k8s_util::events::{Event, EventType};
use kaniop_k8s_util::types::{get_first_as_bool, get_first_cloned};
...
use kanidm_proto::constants::{
    ...
    ATTR_OAUTH2_PREFER_SHORT_USERNAME, ATTR_OAUTH2_RS_ORIGIN, ATTR_OAUTH2_RS_ORIGIN_LANDING,
...
impl KanidmOAuth2Client {
    ...
    async fn update_strict_redirect_url(
        ...
        debug!(msg = "update {ATTR_OAUTH2_STRICT_REDIRECT_URI} attribute");
        if let Some(strict_redirect_url_enabled) = self.spec.strict_redirect_uri {
    ...
    async fn update_disable_pkce(
        ...
        debug!(msg = "update {ATTR_OAUTH2_ALLOW_INSECURE_CLIENT_DISABLE_PKCE} attribute");
    ...
    async fn update_prefer_short_name(
        ...
        debug!(msg = "update {ATTR_OAUTH2_PREFER_SHORT_USERNAME} attribute");
    ...
    async fn update_allow_localhost_redirect(
        ...
        debug!(msg = "update {ATTR_OAUTH2_ALLOW_LOCALHOST_REDIRECT} attribute");
    ...
    fn generate_status(&self, oauth2_opt: Option<Entry>) -> Result<KanidmOAuth2ClientStatus> {
        ...
        let conditions = match oauth2_opt.clone() {
            Some(oauth2) => {
                ...
                let updated_condition = if Some(&self.spec.displayname)
                    ...
                    && Some(&self.spec.origin)
                        == get_first_cloned(&oauth2, ATTR_OAUTH2_RS_ORIGIN_LANDING).as_ref()
                {
                    Condition {
                ...
                let redirect_url_condition =
                    if Some(&self.spec.redirect_url) == oauth2.attrs.get(ATTR_OAUTH2_RS_ORIGIN) {
                        Condition {
                ...
                let strict_condition = self.spec.strict_redirect_uri.as_ref().map(|s| {
                    if Some(s)
                ...
                let disable_pkce_condition = self.spec.allow_insecure_client_disable_pkce.as_ref().map(|disable_pkce| {
                    if Some(disable_pkce)
                        == get_first_as_bool(&oauth2, ATTR_OAUTH2_ALLOW_INSECURE_CLIENT_DISABLE_PKCE).as_ref()
                    {
                        Condition {
        ...
        Ok(KanidmOAuth2ClientStatus {
            ...
            origin: oauth2_opt.and_then(|o| o.attrs.get(ATTR_OAUTH2_RS_ORIGIN).cloned()),
        })
    }
}
