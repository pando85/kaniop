use crate::crd::{KanidmClaimsMap, KanidmOAuth2Client, KanidmOAuth2ClientStatus, KanidmScopeMap};

use futures::future::TryJoinAll;
use kaniop_k8s_util::events::{Event, EventType};
use kaniop_k8s_util::types::{
    compare_with_url, compare_with_urls, get_first_as_bool, get_first_cloned,
};
...
const TYPE_CLAIMS_MAP_UPDATED: &str = "ClaimsMapUpdated";
...
impl KanidmOAuth2Client {
    ...
    async fn update_redirect_url(
        ...
        // TODO: add set redirect URL method to kanidm client
        // delete and recreate to guarantee order
        let urls_to_delete = status
            .origin
            .as_ref()
            .map(|o| {
                o.iter()
        ...
        let delete_futures = urls_to_delete
            .iter()
            .map(|url| kanidm_client.idm_oauth2_client_remove_origin(name, url));

        futures::future::try_join_all(delete_futures)
            .await
            .map_err(|e| {
                Error::KanidmClientError(
                    format!(
                        "failed to delete {ATTR_OAUTH2_RS_ORIGIN} for {name} from {namespace}/{kanidm}",
                        kanidm = self.spec.kanidm_ref.name
                    ),
                    Box::new(e),
                )
        ...
        for url in urls_to_add.iter() {
            kanidm_client
                .idm_oauth2_client_add_origin(name, url)
                .await
                .map_err(|e| {
                    Error::KanidmClientError(
                        format!(
                            "failed to update {ATTR_OAUTH2_RS_ORIGIN} from {namespace}/{kanidm}",
                            kanidm = self.spec.kanidm_ref.name
                        ),
                        Box::new(e),
                    )
    ...
    async fn update_claims_map(
        ...
        let current_claims_map: BTreeSet<_> = status
            .claims_map
            .as_ref()
            .map(|v| v.iter().filter_map(|v| KanidmClaimsMap::from(v)).collect())
            .unwrap_or_default();

        let claims_map: BTreeSet<KanidmClaimsMap> = self
            .spec
            .claims_map
            .clone()
            .unwrap_or_default()
        ...
        let delete_futures = current_claims_map
            ...
            .map(|s| kanidm_client.idm_oauth2_rs_delete_claim_map(name, &s.name, &s.group))
            .collect::<TryJoinAll<_>>();
        ...
        let add_futures = claims_to_add
            .clone()
            .map(|c| {
                kanidm_client.idm_oauth2_rs_update_claim_map(name, &c.name, &c.group, &c.values)
            })
            .collect::<TryJoinAll<_>>();

        let mut unique_claims = HashSet::new();
        let join_strategy_futures = claims_to_add
            .filter(|c| unique_claims.insert(&c.name))
            .map(|c| {
            ...
            Some(oauth2) => {
                ...
                let updated_condition = if Some(&self.spec.displayname)
                    ...
                    && get_first_cloned(&oauth2, ATTR_OAUTH2_RS_ORIGIN_LANDING)
                        .map(|url| compare_with_url(&self.spec.origin, &url))
                        .unwrap_or(false)
                {
                    Condition {
                ...
                // TODO: edition 2024 replace with:
                // if let Some(x) = oauth2.attrs.get(ATTR_OAUTH2_RS_ORIGIN) &&
                //   compare_with_urls(&self.spec.redirect_url, x) {
                let redirect_url_condition = if oauth2.attrs.contains_key(ATTR_OAUTH2_RS_ORIGIN)
                    && compare_with_urls(
                        &self.spec.redirect_url,
                        oauth2.attrs.get(ATTR_OAUTH2_RS_ORIGIN).unwrap(),
                    ) {
                    Condition {
                ...
                let scope_map_condition = self.spec.scope_map.as_ref().map(|scope_map| {
                    let current_scope_map: Option<BTreeSet<_>> = oauth2
                        .attrs
                        .get(ATTR_OAUTH2_RS_SCOPE_MAP)
                        .and_then(|v| v.iter().map(|v| KanidmScopeMap::from(v)).collect());
                    if current_scope_map == Some(scope_map.iter().cloned().collect::<BTreeSet<_>>())
                    {
                        Condition {
                ...
                let sup_scope_map_condition = self.spec.sup_scope_map.as_ref().map(|sup_scope_map| {
                    let current_sup_scope_map: Option<BTreeSet<_>> = oauth2
                        .attrs
                        .get(ATTR_OAUTH2_RS_SUP_SCOPE_MAP)
                        .and_then(|v| v.iter().map(|v| KanidmScopeMap::from(v)).collect());
                    if current_sup_scope_map == Some(sup_scope_map.iter().cloned().collect::<BTreeSet<_>>())
                    {
                        Condition {
                ...
                let claims_map_condition = self.spec.claims_map.as_ref().map(|claims_map| {
                    let current_claims_map: Option<BTreeSet<_>> = oauth2
                        .attrs
                        .get(ATTR_OAUTH2_RS_CLAIM_MAP)
                        .and_then(|v| v.iter().map(|v| KanidmClaimsMap::from(v)).collect());
                    if current_claims_map == Some(claims_map.iter().cloned().collect::<BTreeSet<_>>())
                    {
                        Condition {
