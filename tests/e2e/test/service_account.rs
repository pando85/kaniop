...
async fn service_account_credentials_rotation() {
    ...
    let credentials_secret_name = format!("{}-kanidm-service-account-credentials", name);
    let initial_secret = secret_api
        .get(&credentials_secret_name)
        .await
        .unwrap();
    ...
    let rotated_annotations = rotated_secret
        .metadata
        .annotations
        .as_ref()
        .unwrap();
    ...
    let new_time = new_rotation_time
...
async fn service_account_api_token_rotation() {
    ...
    wait_for(
        sa_api.clone(),
        name,
        is_service_account("ApiTokensUpdated"),
    )
    .await;
    wait_for(sa_api.clone(), name, is_service_account_ready()).await;
