...
use kaniop_oauth2::crd::{KanidmOAuth2Client, OAuth2ClientImageSpec};
...
#[tokio::test]
async fn oauth2_https_image_fetching() {
    let name = "test-oauth2-https-image";
    let s = setup_kanidm_connection(KANIDM_NAME).await;
    let oauth2_spec = json!({
        "kanidmRef": {
            "name": KANIDM_NAME,
        },
        "displayname": "OAuth2 Client with HTTPS Image",
        "redirectUrl": [],
        "origin": format!("https://{name}.example.com"),
        "image": {
    ...
    let oauth2_created = s.kanidm_client.idm_oauth2_rs_get(name).await.unwrap();
    let attrs = oauth2_created.clone().unwrap().attrs;

    assert!(
        attrs.contains_key("oauth2_rs_icon_file_name"),
        "OAuth2 client should have icon file name set"
    );
    assert!(
        attrs.contains_key("oauth2_rs_icon"),
        "OAuth2 client should have icon data set"
    );

    let image_spec = OAuth2ClientImageSpec {
        url: "https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/argo-cd.svg".to_string(),
    };
    oauth2.spec.image = Some(image_spec);
    oauth2_api
        .patch(
            name,
            &PatchParams::apply("e2e-test").force(),
    ...
    let oauth2_image_unchanged = s.kanidm_client.idm_oauth2_rs_get(name).await.unwrap();
    let attrs_unchanged = oauth2_image_unchanged.clone().unwrap().attrs;

    assert!(
        attrs_unchanged.contains_key("oauth2_rs_icon_file_name"),
        "OAuth2 client should still have icon file name"
    );
    assert!(
        attrs_unchanged.contains_key("oauth2_rs_icon"),
        "OAuth2 client should still have icon data"
    );
}
